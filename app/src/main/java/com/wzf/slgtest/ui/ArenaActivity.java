...
public class ArenaActivity extends BaseActivity {
    ...
    public void arenaInfoWithMe(S2C_ArenaGetArenaInfo msgInfo) {
        tvResult.setText("查看我的竞技场排名： \n" + msgInfo.toString());
    }

    @Subscribe(threadMode = ThreadMode.MAIN)
    public void previewRank(S2C_ArenaPreviewRank msgInfo) {
        tvResult.setText("竞技场前100排名： \n" + msgInfo.toString());
    }

    @Subscribe(threadMode = ThreadMode.MAIN)
    public void arenaChallenge(S2C_ArenaChallenge msgInfo) {
        tvResult.setText("挑战结果： \n" + msgInfo.toString());
    }

    @Subscribe(threadMode = ThreadMode.MAIN)
    public void getEveryDayReward(S2C_ArenaGetDailyReward msgInfo) {
        tvResult.setText("每日奖领取： \n" + msgInfo.toString());
    }

    @Subscribe(threadMode = ThreadMode.MAIN)
    public void exchangeReward(S2C_ArenaExchangeReward msgInfo) {
        tvResult.setText("兑换结果： \n" + msgInfo.toString());
    }

    @Subscribe(threadMode = ThreadMode.MAIN)
    public void rewardRecord(S2C_ArenaRewardRecord msgInfo) {
        tvResult.setText("兑换记录： \n" + msgInfo.toString());
    }

    @Subscribe(threadMode = ThreadMode.MAIN)
    public void notifyAlert(S2C_ArenaSweep msgInfo) {
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.btn_getRank:
                SendUtils.sendMsg(ArenaActivity.this, SGMainProto.E_MSG_ID.MsgID_Arena_GetArenaInfo_VALUE, null);
                break;
            case R.id.btn_getTopRank:
                SendUtils.sendMsg(ArenaActivity.this,SGMainProto.E_MSG_ID.MsgID_Arena_PreviewRank_VALUE, null);
                break;
            case R.id.btn_arenaChallenge:
                new InputTextDialog(this, "targetRand") {
                    ...
                    public void sendText(String text) {
                        ...
                        SendUtils.sendMsg(ArenaActivity.this,SGMainProto.E_MSG_ID.MsgID_Arena_Challenge_VALUE, request.build().toByteArray());
                    }
                }.show();
                break;
            case R.id.btn_getEveryDayReward:
                SendUtils.sendMsg(ArenaActivity.this,SGMainProto.E_MSG_ID.MsgID_Arena_GetDailyReward_VALUE, null);
                break;
            case R.id.btn_rewardRecord:
                SendUtils.sendMsg(ArenaActivity.this,SGMainProto.E_MSG_ID.MsgID_Arena_RewardRecord_VALUE, null);
                break;
            case R.id.btn_reward:
                new InputTextDialog(this, "rewardId") {
                    @Override
                    public void sendText(String text) {
                        C2S_ArenaExchangeReward.Builder request = C2S_ArenaExchangeReward.newBuilder();
                        request.setRewardId(StringUtils.stringToInt(text));
                        SendUtils.sendMsg(ArenaActivity.this,SGMainProto.E_MSG_ID.MsgID_Arena_ExchangeReward_VALUE, request.build().toByteArray());
                    }
                }.show();
            ...
            case R.id.btn_sweep:
                new InputTextDialog(this, "targetRand") {
                    @Override
                    public void sendText(String text) {
                        C2S_ArenaSweep.Builder request = C2S_ArenaSweep.newBuilder();
                        request.setTargetRank(StringUtils.stringToInt(text));
                        SendUtils.sendMsg(ArenaActivity.this,SGMainProto.E_MSG_ID.MsgID_Arena_Sweep_VALUE, request.build().toByteArray());
                    }
                }.show();
                break;
            case R.id.btn_buy_challenge_times:
                new InputTextDialog(this, "times") {
                    ...
                    public void sendText(String text) {
                        ...
                        SendUtils.sendMsg(ArenaActivity.this,SGMainProto.E_MSG_ID.MsgID_Store_BuyTimes_VALUE, request.build().toByteArray());
                    }
                }.show();
                break;
            default:
