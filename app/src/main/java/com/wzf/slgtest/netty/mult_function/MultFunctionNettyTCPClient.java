 ...
 * @date 2016/11/9
 */
public abstract class MultFunctionNettyTCPClient implements Runnable{
    private static List<Nio> nios = new CopyOnWriteArrayList<>();
    public TcpHandlerParam param;
    private String ip;
    private int port;
    public MultFunctionNettyTCPClient(String ip, int port, TcpHandlerParam param) {
        this.param = param;
        this.ip = ip;
        this.port = port;
    }
    ...
    private  void connect()
    {
        NioEventLoopGroup  workerGroup = null;
            synchronized (MultFunctionNettyTCPClient.class){
                if(nios.size() > 0){
        ...
        try {
            ...
            bootstrap.handler(new ChannelInitializer<SocketChannel>() {
                ...
                protected void initChannel(SocketChannel ch) throws Exception {
                    ...
                    ch.pipeline().addLast(new MultFunctionNettyTCPClientHandler(param));
