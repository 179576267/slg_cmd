...
public class HeroTempleActivity extends BaseActivity {
    ...
    public void fun(SGPlayerProto.S2C_HeroTempleInitInfo msgInfo) {
        tvResult.setText("英雄圣殿初始化： \n" + msgInfo.toString());
    }

    @Subscribe(threadMode = ThreadMode.MAIN)
    public void fun(SGPlayerProto.S2C_HeroTempleChallenge msgInfo) {
        tvResult.setText("英雄圣殿挑战发起返回:" + msgInfo.toString());
    }

    @Subscribe(threadMode = ThreadMode.MAIN)
    public void fun(SGPlayerProto.S2C_HeroTempleSweep msgInfo) {
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.btn_init:
                SendUtils.sendMsg(SGMainProto.E_MSG_ID.MsgID_HeroTemple_InitInfo_VALUE, null);
                break;
            case R.id.btn_challenge:
                new InputTextDialog(this, "targetLevelId") {
                    @Override
                    public void sendText(String text) {
                        SGPlayerProto.C2S_HeroTempleChallenge.Builder request = SGPlayerProto.C2S_HeroTempleChallenge.newBuilder();
                        request.setTargetLevelId(StringUtils.stringToInt(text));
                        SendUtils.sendMsg(SGMainProto.E_MSG_ID.MsgID_HeroTemple_Challenge_VALUE, request.build().toByteArray());
                    }
                }.show();
                break;
            case R.id.btn_sweep:
                new InputTextDialog(this, "targetLevelId") {
                    @Override
                    public void sendText(String text) {
                        SGPlayerProto.C2S_HeroTempleSweep.Builder request = SGPlayerProto.C2S_HeroTempleSweep.newBuilder();
                        request.setTargetLevelId(StringUtils.stringToInt(text));
                        SendUtils.sendMsg(SGMainProto.E_MSG_ID.MsgID_HeroTemple_Sweep_VALUE, request.build().toByteArray());
                    }
                }.show();
                break;
            default:
