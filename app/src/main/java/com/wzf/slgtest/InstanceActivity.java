...
public class InstanceActivity extends BaseActivity {
    ...
    public void fun1(SGInstanceProto.S2C_GetInstanceInfo msgInfo) {
        tvResult.setText("查看副本进度： \n" + msgInfo.toString());
    }

    @Subscribe(threadMode = ThreadMode.MAIN)
    public void fun2(SGInstanceProto.S2C_PassLevel msgInfo) {
        tvResult.setText("通过某一关卡： \n" + msgInfo.toString());
    }

    @Subscribe(threadMode = ThreadMode.MAIN)
    public void fun3(SGInstanceProto.S2C_ReceiveInstanceAward msgInfo) {
        tvResult.setText("兑换地图宝箱奖励： \n" + msgInfo.toString());
    }

    @Subscribe(threadMode = ThreadMode.MAIN)
    public void fun4(SGInstanceProto.S2C_ReceiveLevelBoxReward msgInfo) {
        tvResult.setText("兑换副本关卡的宝箱： \n" + msgInfo.toString());
    }

    @Subscribe(threadMode = ThreadMode.MAIN)
    public void exchangeReward(SGInstanceProto.S2C_GetLastPassLevel msgInfo) {
    ...
    @OnClick({R.id.btn_my_instance, R.id.btn_pass_level, R.id.btn_receive_award, R.id.btn_level_award, R.id.btn_last_pass_level})
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.btn_my_instance:
                ...
                SendUtils.sendMsg(SGMainProto.E_MSG_ID.MsgID_Instance_GetInstanceInfo_VALUE, request.build().toByteArray());
                break;
            case R.id.btn_pass_level:
                new InputTextDialog(this, "mapId,levelId;stars") {
                    ...
                    public void sendText(String text) {
                        ...
                        SGInstanceProto.C2S_PassLevel.Builder request = SGInstanceProto.C2S_PassLevel.newBuilder().
                                setChapterId(mapId)
                                .setLevelId(levelId)
                                .setStarts(stars);
                        SendUtils.sendMsg(SGMainProto.E_MSG_ID.MsgID_Instance_PassLevel_VALUE, request.build().toByteArray());
                    }
                }.show();
                tvResult.setText("");
                break;
            case R.id.btn_receive_award:
                new InputTextDialog(this, "mapId;rewardId") {
                    ...
                    public void sendText(String text) {
                        ...
                        SendUtils.sendMsg(SGMainProto.E_MSG_ID.MsgID_Instance_ReceiveAward_VALUE, request.build().toByteArray());
                    }
                }.show();
                break;
            case R.id.btn_level_award:
                new InputTextDialog(this, "mapId;levelId") {
                    ...
                    public void sendText(String text) {
                        ...
                        int mapId = StringUtils.stringToInt(ids[0]);
                        int levelId = StringUtils.stringToInt(ids[1]);
                        SGInstanceProto.C2S_ReceiveLevelBoxReward.Builder request =  SGInstanceProto.C2S_ReceiveLevelBoxReward.newBuilder();
                        request.setChapterId(mapId);
                        request.setLevelId(levelId);
                        SendUtils.sendMsg(SGMainProto.E_MSG_ID.MsgID_Instance_ReceiveLevelBoxReward_VALUE, request.build().toByteArray());
                    }
                }.show();
                break;
            case R.id.btn_last_pass_level:
                SendUtils.sendMsg(SGMainProto.E_MSG_ID.MsgID_Instance_GetLastPassLevel_VALUE, null);
                break;
            default:
                break;
        }
